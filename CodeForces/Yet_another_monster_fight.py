'''
   Vasya is a sorcerer that fights monsters. Again. There are n
 monsters standing in a row, the amount of health points of the i
-th monster is ai
.
...
Firstly, Vasya chooses an index i
 of some monster (1≤i≤n
) and the initial power of the spell x
. Then the spell hits monsters exactly n
 times, one hit per monster. The first target of the spell is always the monster i
. For every target except for the first one, the chain lightning will choose a random monster who was not hit by the spell and is adjacent to one of the monsters that already was hit. So, each monster will be hit exactly once. The first monster hit by the spell receives x
 damage, the second monster receives (x−1)
 damage, the third receives (x−2)
 damage, and so on.

Vasya wants to show how powerful he is, so he wants to kill all the monsters with a single chain lightning spell. The monster is considered dead if the damage he received is not less than the amount of its health points. On the other hand, Vasya wants to show he doesn't care that much, so he wants to choose the minimum initial power of the spell x
...
The second line of the input contains n
 ...
 is the amount of health points of the i
-th monster.
